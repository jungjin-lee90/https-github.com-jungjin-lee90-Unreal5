...
#include "MyBTService_EnemyDetect.h"
#include "MyAIController.h"
#include "MyCharacter.h"
#include "BehaviorTree/BlackboardComponent.h"
#include "DrawDebugHelpers.h"

UMyBTService_EnemyDetect::UMyBTService_EnemyDetect()
{
	NodeName = TEXT("Detect_Enemy");
	Interval = 1.0f;
}

void UMyBTService_EnemyDetect::TickNode(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory, float DetalSeconds)
{
	Super::TickNode(OwnerComp, NodeMemory, DetalSeconds);

	UE_LOG(LogTemp, Warning, TEXT("UMyBTService_EnemyDetect::TickNode %s"), *NodeName);
	...
	if (bResult)
	{
		for (auto const& OverlapResult : OverlapResults)
		{
			...
			if (MyCharacter && MyCharacter->GetController()->IsPlayerController()) // 플레이어가 컨트롤 하는 MyCharacter만 처리
			{
				...
				{
					OwnerComp.GetBlackboardComponent()->SetValueAsObject(AMyAIController::EnemyTargetKey, MyCharacter); // 블랙보드 TargetKey에 값 셋팅
	...
	OwnerComp.GetBlackboardComponent()->SetValueAsObject(AMyAIController::EnemyTargetKey, nullptr);
